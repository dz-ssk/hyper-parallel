...
# ============================================================================
"""Translate the reference YAML into an actual HF V3.2 configuration."""
...
def load_reference(path: str | Path) -> dict[str, Any]:
    """Read and validate the supported reference topology and compute policy.

    Args:
        path: Unmodified reference training YAML.
    """
    document = yaml.safe_load(Path(path).read_text(encoding="utf-8"))
    model = document["model"]["model_config"]
    parallel = document["parallel_config"]
    expected = {"data_parallel": 1, "model_parallel": 8, "expert_parallel": 8,
                "pipeline_stage": 1, "context_parallel": 1, "micro_batch_num": 1,
                "use_seq_parallel": True}
    for name, value in expected.items():
        if parallel[name] != value:
            raise ValueError(f"Reference recipe requires {name}={value}, got {parallel[name]}")
    expected_model = {"seq_length": 262144, "params_dtype": "float32", "compute_dtype": "bfloat16",
                      "multi_latent_attention": True, "use_flash_attention": True,
                      "mla_qkv_concat": False, "add_bias_linear": False, "hidden_act": "silu",
                      "attention_dropout": 0.0, "hidden_dropout": 0.0, "rotary_scaling_factor": 1,
...
def make_hf_config(document: dict[str, Any]) -> DeepseekV32Config:
    """Map model dimensions without silently taking production-sized HF defaults.
    ...
    config = DeepseekV32Config(
        **{name: model[name] for name in names},
        ...
        tie_word_embeddings=False, use_cache=False, attention_bias=False, mlp_bias=False,
        layer_types=["full_attention"] * model["num_hidden_layers"],
    )
    config.architectures = ["DeepseekV32ForCausalLM"]
    # Hyper MLA reads this explicit flag; HF V3.2 hardcodes interleaving in its forward.
